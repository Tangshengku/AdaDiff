...
class ScoreModel(object):
    ...
    def noise_pred(self, xt, t, **kwargs):
        pred, inner_states, lte = self.predict(xt, t, **kwargs)
        if self.pred == 'noise_pred':
            ...
            raise NotImplementedError
        return noise_pred, inner_states, lte
    ...
    def score(self, xt, t, **kwargs):
        ...
        noise_pred, inner_state, lte = self.noise_pred(xt, t, **kwargs)
        return stp(-cum_beta.rsqrt(), noise_pred), inner_state, lte
...
class ReverseSDE(object):
    ...
    def drift(self, x, t, **kwargs):
        ...
        score, inner_state, _ = self.score_model.score(x, t, **kwargs)
...
def LSimple(score_model: ScoreModel, x0, _step, pred='noise_pred', **kwargs):
    ...
    if pred == 'noise_pred':
        noise_pred, inner_pred, lte = score_model.noise_pred(xt, t, **kwargs)
